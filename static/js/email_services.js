...
// 状态
let outlookServices = [];
let customServices = [];  // 合并 moe_mail + yyds_mail + temp_mail + cloudmail + duck_mail + freemail + imap_mail
...
// 加载自定义邮箱服务（moe_mail + yyds_mail + temp_mail + cloudmail + duck_mail + freemail + imap_mail 合并）
...
function applyTempmailConfig(settings = {}) {
    ...
    if (settings.yyds_mail) {
        elements.yydsMailApi.value = settings.yyds_mail.api_url || '';
