...
// 状态
let outlookServices = [];
let customServices = [];  // 合并 moe_mail + tempmail_builtin + yyds_mail + temp_mail + cloudmail + duck_mail + luckmail + freemail + imap_mail
...
// 加载自定义邮箱服务（moe_mail + tempmail_builtin + yyds_mail + temp_mail + cloudmail + duck_mail + luckmail + freemail + imap_mail 合并）
...
// 加载临时邮箱配置
function applyTempmailConfig(settings = {}) {
    ...
    if (settings.yyds_mail) {
        ...
        elements.yydsApiKey.dataset.hasKey = settings.yyds_mail.has_api_key ? 'true' : 'false';
        elements.yydsApiKey.placeholder = settings.yyds_mail.has_api_key ? '已设置，留空保持不变' : 'AC-your_api_key';
    }
}
...
async function handleAddCustom(e) {
    ...
    if (subType === 'yyds_mail' && (!config.base_url || !config.api_key)) {
...
async function handleSaveTempmail(e) {
    ...
    try {
        ...
        const hasSavedYydsKey = elements.yydsApiKey.dataset.hasKey === 'true';
        if (elements.yydsEnabled.checked && !yydsApiKey && !hasSavedYydsKey) {
